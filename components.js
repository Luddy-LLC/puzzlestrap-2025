import { getCookie, setCookie, deleteCookie, camelise, loginNeeded, checkAnswer, loginUser, logoutUser } from "./logic/helper.js";
...
class BackButton extends HTMLElement {
    ...
    render() {
        const c = `<nav class="desktop-nav"><ul role="menubar" aria-label="functions"><li role="presentation"><a href="/" class="nav-button" role="menuitem"><svg xmlns="http://www.w3.org/2000/svg" width="25" height="25" fill="currentColor" viewBox="0 0 16 16"><path d="M12.17 9.53c2.307-2.592 3.278-4.684 3.641-6.218.21-.887.214-1.58.16-2.065a3.6 3.6 0 0 0-.108-.563 2 2 0 0 0-.078-.23V.453c-.073-.164-.168-.234-.352-.295a2 2 0 0 0-.16-.045 4 4 0 0 0-.57-.093c-.49-.044-1.19-.03-2.08.188-1.536.374-3.618 1.343-6.161 3.604l-2.4.238h-.006a2.55 2.55 0 0 0-1.524.734L.15 7.17a.512.512 0 0 0 .433.868l1.896-.271c.28-.04.592.013.955.132.232.076.437.16.655.248l.203.083c.196.816.66 1.58 1.275 2.195.613.614 1.376 1.08 2.191 1.277l.082.202c.089.218.173.424.249.657.118.363.172.676.132.956l-.271 1.9a.512.512 0 0 0 .867.433l2.382-2.386c.41-.41.668-.949.732-1.526zm.11-3.699c-.797.8-1.93.961-2.528.362-.598-.6-.436-1.733.361-2.532.798-.799 1.93-.96 2.528-.361s.437 1.732-.36 2.531Z"></path><path d="M5.205 10.787a7.6 7.6 0 0 0 1.804 1.352c-1.118 1.007-4.929 2.028-5.054 1.903-.126-.127.737-4.189 1.839-5.18.346.69.837 1.35 1.411 1.925"></path></svg><span class="btn-txt">Back to Ship</span></a></li><li role="presentation"><h1 class="puzzle-title">${this.getAttribute("label")}</h1></li></ul></nav><nav class="mobile-nav"><ul role="menubar" aria-label="functions"><div class="mn-top"><li role="presentation"><a href="/" class="nav-button" role="menuitem"><svg xmlns="http://www.w3.org/2000/svg" width="25" height="25" fill="currentColor" viewBox="0 0 16 16"><path d="M12.17 9.53c2.307-2.592 3.278-4.684 3.641-6.218.21-.887.214-1.58.16-2.065a3.6 3.6 0 0 0-.108-.563 2 2 0 0 0-.078-.23V.453c-.073-.164-.168-.234-.352-.295a2 2 0 0 0-.16-.045 4 4 0 0 0-.57-.093c-.49-.044-1.19-.03-2.08.188-1.536.374-3.618 1.343-6.161 3.604l-2.4.238h-.006a2.55 2.55 0 0 0-1.524.734L.15 7.17a.512.512 0 0 0 .433.868l1.896-.271c.28-.04.592.013.955.132.232.076.437.16.655.248l.203.083c.196.816.66 1.58 1.275 2.195.613.614 1.376 1.08 2.191 1.277l.082.202c.089.218.173.424.249.657.118.363.172.676.132.956l-.271 1.9a.512.512 0 0 0 .867.433l2.382-2.386c.41-.41.668-.949.732-1.526zm.11-3.699c-.797.8-1.93.961-2.528.362-.598-.6-.436-1.733.361-2.532.798-.799 1.93-.96 2.528-.361s.437 1.732-.36 2.531Z"></path><path d="M5.205 10.787a7.6 7.6 0 0 0 1.804 1.352c-1.118 1.007-4.929 2.028-5.054 1.903-.126-.127.737-4.189 1.839-5.18.346.69.837 1.35 1.411 1.925"></path></svg><span class="btn-txt">Back to Ship</span></a></li><li role="presentation"><h1 class="puzzle-title">${this.getAttribute("label")}</h1></li></div></ul></nav>`
        this.innerHTML = c;
    }
    connectedCallback() { this.render(); }
    attributeChangedCallback() { this.render(); }
}
customElements.define("mp-back", BackButton);
...
class OffCanvas extends HTMLElement {
    ...
    connectedCallback() {
        ...
        document.querySelectorAll(".offcanvas-panel")[0].addEventListener("toggle", (e) => {
            const accentColour = window.getComputedStyle(document.body).getPropertyValue('--accent-very-dark');
            e.newState === "open" ? meta.content = accentColour : meta.content = metaColour;
        })
    };
    attributeChangedCallback() { this.render(); }
}
...
class Solve extends HTMLElement {
    ...
    render() {
        if (!loginNeeded()) {
            const teamCookie = getCookie('team'), nameCookie = getCookie('name');
            this.innerHTML = `
            <div class="mp-offcanvas">
                <div class="offcanvas-panel" role="dialog"
                        ...
                        <form id="check-answer" class="end mt15">
                            ...
                            <div class="form-group" hidden>
                                <input class="mp-input" required="required" id="name" placeholder="Enter your name"
                                    name="name" type="text" autocomplete="off"  value="${nameCookie}">
                                <label for="name">Your Name</label>
                            </div>
                            <div class="form-group" hidden>
                                <input class="mp-input" required="required" id="team" placeholder="Enter your team name"
                                    name="team" type="text" autocomplete="off" value="${teamCookie}">
                                <label for="team">Team Name</label>
                            </div>
                            <div class="form-group" hidden>
                                <input class="mp-input" required="required" id="puzzle" placeholder="Enter puzzle id"
                                    name="puzzle" type="text" autocomplete="off" value="${document.getElementsByTagName('html')[0].getAttribute('data-puzzle')}">
                    ...
                    <div class="oc-footer">
                        <p>Hello, <span class="bi">${nameCookie}</span>! You're submitting as a member of team <span class="bi">${teamCookie}</span>. <button class="plain underline" id="switch-user">Switch User</button></p>
                    </div>
                </div>
            </div>
            `;
            ...
            <div class="mp-offcanvas">
                <div class="offcanvas-panel" role="dialog"
                    popover id="solve-menu" aria-labelledby="solvetitle">
                    <div class="oc-header"><svg width="40px" height="40px" aria-hidden="true">
                            <use href="static/icons.svg#mp-person"
                                xlink:href="static/icons.svg#mp-person"></use>
                        </svg>
                        <h1 id="solvetitle">Sign In</h1><button
                            popovertarget="solve-menu" class="plain" aria-label="Close Menu"><svg width="40px" height="40px"
                                aria-hidden="true">
                                <use href="static/icons.svg#mp-x" xlink:href="static/icons.svg#mp-x"></use>
                            </svg></button>
                    ...
                    <div class="oc-body">
                        <mp-alert label="Sign in to submit an answer..." type="info" style="display: block"></mp-alert>
                        <form id="login-form" class="end mt15">
                            <div class="form-group">
                                <input class="mp-input" required="required" id="name" placeholder="Enter your name"
                                    name="name" type="text" autocomplete="off" value="">
                                <label for="name">Your Name</label>
                            </div>
                            <div class="form-group">
                                <input class="mp-input" required="required" id="team" placeholder="Enter your team name"
                                    name="team" type="text" autocomplete="off" value="">
                                <label for="team">Team Name</label>
                            </div>
                    ...
                    <div class="oc-footer">
                        <p>Use an existing Mega Puzzle team name to play with friends or enter a new one to start a team!</p>
                    </div>
                </div>
            </div>
            `;
        }
        if (this.hasAttribute("reopen") && this.getAttribute("reopen") === "true") {
            document.getElementById('solve-menu').showPopover();
            this.removeAttribute("reopen");
        }
        try {document.getElementById('check-answer').addEventListener("submit", answerSubmission);} catch {}
        try{document.getElementById("login-form").addEventListener("submit",(e=>{e.preventDefault(),loginUser(document.getElementById("team").value,document.getElementById("name").value),this.setAttribute("reopen","true"),this.render()}))}catch{}
        try{document.getElementById("switch-user").addEventListener("click",(()=>{logoutUser(),this.setAttribute("reopen","true"),this.render()}))}catch{}
    }
    connectedCallback() {
        this.render();
        ...
        function answerSubmission(event) {
            ...
            checkAnswer(document.getElementById('team').value, document.getElementById('name').value, document.getElementById('puzzle').value, document.getElementById('answer').value.toLowerCase()).then((response) => {
                c.style.display = 'none', i.style.display = 'none', t.style.display = 'none';
                try {
                    "correct" == response.result ? c.style.display = "block" : "incorrect" == response.result ? i.style.display = "block" : t.style.display = "block";
                } catch (e) {
...
class Music extends HTMLElement {
            ...
            <div class="offcanvas-panel" role="dialog"
                ...
                <div class="oc-header"><svg width="40px" height="40px" aria-hidden="true">
                    ...
                    <h1 id="hintstitle">Music</h1><button
                        popovertarget="music-menu" class="plain" aria-label="Close Menu"><svg width="40px" height="40px"
                            aria-hidden="true">
                            <use href="static/icons.svg#mp-x" xlink:href="static/icons.svg#mp-x"></use>
                        </svg></button>
                </div>
                <mp-musicinfo is-playing="false"></mp-musicinfo>
                <div id="mp-music-footer" class="oc-footer">
                    <p>${footerMessage ?? ""}</p>
                </div>
            </div>
        </div>
    ...
    attributeChangedCallback(name, oldValue, newValue) {
        const songInfo = {
            "song": document.getElementById('song-title').innerText,
            "artist": document.getElementById('song-artist').innerText,
            "file": document.getElementById('audio-player').src
        }
        const footerMessage = document.getElementById('mp-music-footer').innerText;
        songInfo.song != "none" ? this.render(songInfo, footerMessage) : this.render({ "song": "Silence.", "artist": "There's nothing like 4:33.", "file": "" }, footerMessage);
    }
    connectedCallback() {
        fetch('puzzles.json')
            .then(response => response.json())
            .then((data) => {
                ...
                fetch('logic/shipdialogue.json')
                    ...
                    .then((data) => {
                        ...
                        song != "none" ? this.render(song, footerMessage) : this.render({ "song": "Silence.", "artist": "There's nothing like 4:33.", "file": "" }, footerMessage);
                    }).catch(error => console.log(error));
...
class MusicInfo extends HTMLElement {
    static observedAttributes = ["is-playing"];
    constructor() { super(); }
    render(songInfo, isPlaying) {
        this.innerHTML = `
                <div class="oc-body music">
                    <div class="flex-c">
                    <div class="flex-r">
                        <svg width="20px" height="20px" aria-hidden="true">
                            ${isPlaying === "true" ? `<use id="playing-indicator" href="static/icons.svg#mp-eq-animated" height="20px" xlink:href="static/icons.svg#mp-eq-animated"></use>` : `<use id="playing-indicator" href="static/icons.svg#mp-eq" height="20px" xlink:href="static/icons.svg#mp-eq"></use>`}
                        </svg>
                        <p class="song-small caps smaller">${isPlaying === "true" ? "Now Playing" : "Up Next"}</p>
                    </div>
                    
                    <p id="song-title" class="song-large">${songInfo.song ?? ""}</p>
                    <p id="song-artist" class="song-small">${songInfo.artist ?? ""}</p>
                    </div>
                    <button class="plain" id="playpause-button-area">
                        <svg id="playpause-button" width="40px" height="40px" aria-label="${isPlaying ? "Pause Music" : "Play Music"}">
                            <use id="playpause-icon" href="${isPlaying === "false" ? "static/icons.svg#mp-play-circle" : "static/icons.svg#mp-pause-circle"}" xlink:href="${this.getAttribute("is-playing") ? "static/icons.svg#mp-play-circle" : "static/icons.svg#mp-pause-circle"}"></use>
                        </svg>
                    </button>
                </div>`;
    }
    attributeChangedCallback(name, oldValue, newValue) {
        ...
        songInfo.song != "none" ? this.render(songInfo, this.getAttribute('is-playing')) : this.render({ "song": "Silence.", "artist": "There's nothing like 4:33.", "file": "" }, this.getAttribute('is-playing'));
    }
    connectedCallback() {
        fetch('puzzles.json')
            .then(response => response.json())
            .then((data) => {
                ...
                fetch('logic/shipdialogue.json')
                    ...
                    .then((data) => {
                        const footerMessage = data.music[~~(Math.random() * data.music.length)];
                        song != "none" ? this.render(song, this.getAttribute('is-playing')) : this.render({ "song": "Silence.", "artist": "There's nothing like 4:33.", "file": "" }, this.getAttribute('is-playing'));
                        const audioElement = document.getElementById('audio-player');
                        this.addEventListener("click", (e) => {
                            if (e.target.id === "playpause-button" || e.target.id === "playpause-icon") {
                                audioElement.paused ? this.setAttribute('is-playing', 'true') : this.setAttribute('is-playing', 'false');
                        ...
                        audioElement.play();
                        document.addEventListener("click", function allClicksAudioStarter(e) {
                            console.log('paying audio')
                            try {
                                audioElement.play();
                                document.removeEventListener("click", allClicksAudioStarter);
                            } catch { }
...
customElements.define("mp-musicinfo", MusicInfo);