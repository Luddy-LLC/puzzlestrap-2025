...
class Hints extends HTMLElement {
    ...
    connectedCallback() {
        var h = "";
        fetch('puzzles.json').then((response1)=>response1.json()).then((data)=>{
            const hints = data[document.getElementsByTagName('html')[0].getAttribute('data-puzzle')].hints;
            for(const hint in hints){
                h += `<li>${hints[hint]}</li>`;
            }
            fetch('logic/shipdialogue.json').then((response1)=>response1.json()).then((data)=>{
...
class Solve extends HTMLElement {
    ...
    connectedCallback() {
        ...
        function preventDefaultSubmission(event) {
            ...
            (0,_logic_check_js__WEBPACK_IMPORTED_MODULE_0__.checkAnswer)(document.getElementById('team').value, document.getElementById('name').value, document.getElementById('puzzle').value, document.getElementById('answer').value.toLowerCase()).then((r)=>{
...
function autoFill() {
    document.getElementById('team').value = "Whom Whom";
    document.getElementById('name').value = "Reece";
...
//# sourceMappingURL=main.65112196cb2210eeeadf.hot-update.js.map